...
const { isSameDay, addDays, format, subDays } = require('date-fns');
...
module.exports = async (store, facebook) => {
  ...
  // const long_live_access_token = await facebookAPI.getLonglivedAccessToken(short_access_token);
  ...
  try {
    while (true) {
      ...
      const date_start = !results.length
        ...
        : utcToZonedTime(
            addDays(results[0].date_start.date, 1),
            results[0].date_start.timezone
          )
      ;
      ...
      const {upsertedCount, modifiedCount} = await mongo.client
        ...
        .bulkWrite(
          insights.data.map((row) => {
            ...
            const newRow = {
              ...row,
              purchases_count: purchasesCount && purchasesCount.value
                ? `${purchasesCount.value}`
                : null,
              purchases_value: purchasesValue && purchasesValue.value
      ...
      console.log(
        '[Facebook] rows upserted/updated: %d/%d.',
        upsertedCount,
        modifiedCount
      );

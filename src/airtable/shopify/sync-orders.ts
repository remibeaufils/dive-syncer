...
const getFreshestDateInAirtable = async (base, table) => {
    const records = await base(table)
        .select({
            maxRecords: 1,
            sort: [{ field: 'order_updated_at', direction: 'desc' }],
        })
        .all();

    return records?.[0]?.get('order_updated_at');
};

const avoidDuplicate = async (base, table, freshestDateInAirtable) => {
    const records = await base(table)
        .select({
            filterByFormula: `{order_updated_at} = '${freshestDateInAirtable}'`,
        })
        .all();
...
const getDataFromMongo = async (store, lastDate, collection, timezone) => {
    try {
        ...
        const results = await mongo.client
            ...
            .find({
                store_id: store.id,
                'created_at.date': { $gte: toDate(DEFAULT_MIN_DATE, { timeZone: timezone }) },
                'updated_at.date': { $gt: minDate },
            })
            .sort({ 'updated_at.date': 1 })
            .limit(6000)
            .toArray();
...
export default async (table, collection, store, base, timezone) => {
    console.log('[Airtable Shopify] - Sync orders - begin');
    ...
    try {
        ...
        console.log('[Airtable Shopify] - Sync orders - sync airtable');

        await Promise.all(requests);
    } catch (error) {
        console.log('\x1b[31mError: %s\x1b[0m', error);
    } finally {
        console.log('[Airtable Shopify] - Sync orders - end');
    }
};
